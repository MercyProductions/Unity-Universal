...
namespace Utils
{
	void CreateConsole() {
		...
		SetConsoleTitle(Variables::AppName);
		FILE* f;
	...
	{
		if (Variables::System::ScreenSize.x <= 0.0f || Variables::System::ScreenSize.y <= 0.0f) {
			return false;
		}
		...
		if (buffer.z > 0.0f) 
		{
			...
			return screen.x > 0.0f || screen.y > 0.0f;
		}

		return false;
	}

	void MouseMove(float tarx, float tary, float X, float Y, float smooth)
	{
	...
	Unity::CGameObject* GetNearestPlayer(const std::vector<Unity::CGameObject*>& list, Unity::CGameObject* localplayer)
	{
		if (!localplayer || !localplayer->GetTransform())
			return nullptr;

		Unity::CGameObject* nearestPlayer = nullptr;
		float nearestDistance = FLT_MAX;
		const Unity::Vector3 localPos = localplayer->GetTransform()->GetPosition();
		...
		for (size_t i = 0; i < list.size(); i++)
		{
			...
			Unity::CTransform* currentTransform = currentPlayer->GetTransform();
			if (!currentTransform)
				continue;

			float distance = GetDistance(localPos, currentTransform->GetPosition());
	...
	void DrawLuaEditor() {
		...
		if (!ImGui::Begin("Aegis Lua Editor", nullptr)) {
			ImGui::End();
			return;
		}
	...
	void DrawInspector() {
		...
		if (ImGui::Button("Update##class")) {
			if (!components.empty() && component_current_idx >= 0 && component_current_idx < static_cast<int>(components.size())) {
		...
		if (ImGui::Button("Update##Methods")) {
			if (!classes.empty() && class_current_idx >= 0 && class_current_idx < static_cast<int>(classes.size())) {
				methods = Dumper::DumpMethodsString(current_comp, classes[class_current_idx]);
				method_current_idx = 0;
			}
		}
	...
	void ObjectsCache(std::vector<Unity::CGameObject*>* originalList, const char* CName, const char* CNameLocalPlayerUniqueComponent = nullptr, int maxResults = 512)
	{
		...
		for (int i = 0; i < list->m_uMaxLength; i++)
		{
			...
			if (static_cast<int>(originalList->size()) >= maxResults)
				break;
		}
